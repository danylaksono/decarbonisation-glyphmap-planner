...
//(so they can be stacked without fear of running over 1 or height)
  ...
  this.maxAtTime = (time) => stacked ?
      d3.sum(this.categories().map( c => this.data(c,time))) :
      d3.max(this.categories().map( c => this.data(c,time)));
  this.maxAllTime = () => d3.max( d3.range(this.timeCount()).map( t => this.maxAtTime(t)));
  ...
  this.draw = (ctx, x, y, w, h) => {
    ...
    let norm = collection != null ? collection.norm : this.max();

    let colors = d3.scaleOrdinal(d3.schemeTableau10).domain(d3.range(Object.keys(data).length));

    let yOffset = d3.range(maxTime).map( v => 0); //zeros
    this.categories().map ( (c,cindex) => {

        ctx.beginPath();
        ctx.moveTo(x + (this.timeCount()-1)*xInterval, y + yOffset[this.timeCount() - 1]);
