...
// display("regular geodata with census data");
...
<!-- ------------------ # Gridded-Glyphmap Data Functions ------------------  -->

```js
const osgb = new OSGB();
let clone = turf.clone(regular_geodata_withcensus);
...
const regularGeodataCensusWgs84 = clone;

// display("regularGeodataCensusWgs84");
// display(regularGeodataCensusWgs84);
```

```js
const osgb = new OSGB();
let clone = turf.clone(grid_geodata_withcensus);
turf.coordEach(clone, (currentCoord) => {
  const newCoord = osgb.toGeo(currentCoord);
  currentCoord[0] = newCoord[0];
  currentCoord[1] = newCoord[1];
});
const gridGeodataCensusWgs84 = clone;

// display("gridGeodataCensusWgs84");
// display(gridGeodataCensusWgs84);
```
...
const regularGeodataLookupWGS84 = _.keyBy(
  regular_geodata_withcensus.features.map((feat) => {
...
const gridGeodataLookupWGS84 = _.keyBy(
  grid_geodata_withcensus.features.map((feat) => {
    const transformedGeometry = transformGeometry(feat.geometry);
    const centroid = turf.getCoord(turf.centroid(transformedGeometry));
    return {
...
for (const key of Object.keys(gridGeodataLookupWGS84)) {
  if (regularGeodataLookupWGS84[key] && gridGeodataLookupWGS84[key]) {
    flubbers[key] = flubber.interpolate(
      turf.getCoords(regularGeodataLookupWGS84[key])[0],
      turf.getCoords(gridGeodataLookupWGS84[key])[0],
      { string: false }
    );
  }
}
const tweenWGS84Lookup = _.mapValues(flubbers, (v, k) => {
  const feat = turf.multiLineString([v(t)], { code: k });
  feat.centroid = turf.getCoord(turf.centroid(feat.geometry));
  return feat;
});

// display("tweenWGS84Lookup");
// display(tweenWGS84Lookup);
```
...
```js
t; //causes code to run whenever the slider is moved
// display(t);
morphGlyphMap.setGlyph({ discretiserFn: valueDiscretiser(tweenWGS84Lookup) });
```
...
const glyphMapSpecWgs84 = {
  ...
  initialBB: turf.bbox(regularGeodataCensusWgs84), //use the WGS84 extent
  getLocationFn: (row) => regularGeodataLookupWGS84[row.code]?.centroid, //use the WGS84 location (needed for the gridded glyphmaps)
};
...
const morphGlyphMap = createMorphGlyphMap(800, 600);
```

```js
// function morphGlyphMap(width, height) {
//   return glyphMap({
//     // ...glyphMapSpec, // this is for non-mercator CRS
//     ...glyphMapSpecWgs84, //takes the base spec...
//     width: width,
//     height: height,
//     //discretiserFn: valueDiscretiser(intermed) //uses the polyon lookup for spatialunits
//   });
// }
// display("morphGlyphMap");
```
